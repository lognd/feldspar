...
to `feldspar.elec.signal_integrity`; this module becomes a transparent alias
(via `sys.modules`, not `import *`, so every attribute -- not just
`__all__` -- stays reachable through the old path) so
`feldspar.library.signal_integrity` keeps working exactly as
before (behavior-preserving, D227)."""

import sys

from feldspar.elec import signal_integrity as _real  # noqa: F401
