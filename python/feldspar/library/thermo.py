...
to `feldspar.thermo.properties`; this module becomes a transparent alias
(via `sys.modules`, not `import *`, so every attribute -- not just
`__all__` -- stays reachable through the old path) so
`feldspar.library.thermo` keeps working exactly as
before (behavior-preserving, D227)."""

import sys

from feldspar.thermo import properties as _real  # noqa: F401
