...
to `feldspar.mech.plate`; this module becomes a transparent alias
(via `sys.modules`, not `import *`, so every attribute -- not just
`__all__` -- stays reachable through the old path) so
`feldspar.library.plate` keeps working exactly as
before (behavior-preserving, D227)."""

import sys

from feldspar.mech import plate as _real  # noqa: F401
