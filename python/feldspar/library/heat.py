...
to `feldspar.heat.closed_form`; this module becomes a transparent alias
(via `sys.modules`, not `import *`, so every attribute -- not just
`__all__` -- stays reachable through the old path) so
`feldspar.library.heat` keeps working exactly as
before (behavior-preserving, D227)."""

import sys

from feldspar.heat import closed_form as _real  # noqa: F401
