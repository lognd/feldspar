...
| FINV-9 | Parallel == serial: bit-identical Solutions at any thread count | WO-15: `feldspar_core::propagation::hull_from_results` folds a fixed, corner-ordered result set (order-independent min/max, Rust unit-tested against shuffled input); `feldspar.plan.parallel.parallel_corner_sweep` and `feldspar.calib.harness.calibrate`'s `thread_count` dispatch via `ThreadPoolExecutor` (order-preserving `map`), `thread_count<=1` the always-present serial fallthrough; `tests/unit/test_parallel_determinism.py` diffs serial vs. parallel at thread counts 1/2/4/8; CI determinism job runs both (AD-12c) |
